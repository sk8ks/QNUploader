...
const moxie = require('./moxie.min');
const plupload = require('./plupload.min');
    ...
    function QiniuJsSDK() {
        ...
        this.uploader = function (op) {
            ...
            var getUpHosts = function (uptoken) {
                ...
                // 源代码里第三个参数为false，表示同步，这里改为true，异步
            ...
            var getNewUpToken = function (file, up) {
                if (op.uptoken) {
                    // that.token = op.uptoken;
                    that.token = up.getOption().uptoken;
                    ...
                    that.token = up.getOption().uptoken_func(file);
                    logger.debug("get new uptoken: ", that.token);
                } else {
            ...
            // 新增函数获取token
            that.token_handler = typeof op.init.Token === 'function' ? op.init.Token : '';
            ...
            uploader.bind('Error', (function (_Error_Handler) {
                return function (up, err) {
                    ...
                    //     console.log(err);
                    //     var req_id = matchedGroups[2] ? matchedGroups[2].replace(/[\r\n]/g,"") : '';
