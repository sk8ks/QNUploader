...
const qiniuConf = {
	...
	// uptoken_url: '',             // Ajax请求uptoken的Url，会自动触发，
	...
	const uploader = Qiniu.uploader({
		...
		init: {
			...
			FileFiltered: function(up, file) {
				...
				if (!isType) {
					error = { msg: `文件'${file.name}'格式不符，请重新上传` };
					console.warn(`文件'${file.name}'格式不符，请重新上传`);
				}

				file.canUpload = isType;
				isFunc(conf.fileFiltered) && conf.fileFiltered(up, file, error);
			},
			...
			FilesAdded: function(up, files) {
				...
				if (maxFiles <= 0) {
					...
					if (fileList.length > 0) {
						if (fileList.length > maxFiles) {
							error = { msg: `文件过多，一次最多上传${maxFiles}个文件` };
				...
				// 用户自定义的回调
				isFunc(conf.filesAdded) && conf.filesAdded(up, files, error);
			},
			/**
			 * @desc 上传前对上传流程做最后校验
			 * @param  {Object} up   当前上传实例
			...
			BeforeUpload: function(up, file) {
				isFunc(conf.beforeUpload) && conf.beforeUpload(up, file);
			},
			/**
			 * @desc 上传中...
             * @param  {Object} up   当前上传实例
			 ...
			 * @desc    上传过程报错
			 * @param  {Object} up   上传对象
			 * @param  {Object} err     上传报错对象
			 * @param  {Object} errTip  上传错误信息
			 */
